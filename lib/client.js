window.__ModuleLoader__.load({
	id: (() => {
		const fallback = "@webwalkerhq/dsh-replay-lab";
		try {
			...
			const id = decodeURIComponent(pathname.slice(start + 9, -10));
	...
	factory: (require) => {
		...
		var ReplayLabController = class {
			...
			async summarize(experimentId, prompt) {
				await this.request("POST", "/summarize", {
					sessionId: this.requireSessionId(),
					experimentId,
					prompt
				});
			}
			async renderDashboard(experimentId, promptId, prompt) {
		...
		//#endregion
		//#region src/dashboard-payload.ts
		function unique$1(values) {
			return [...new Set(values.filter((value) => value.length > 0))];
		}
		function slice(id, kind, label, evidence, fallback) {
			const surfaces = evidence?.requestSurfaces ?? [];
			const route = surfaces.length > 0 ? unique$1(surfaces.map((surface) => `${surface.provider} / ${surface.model}`)).join(" → ") : fallback === void 0 ? null : `${fallback.provider} / ${fallback.model}`;
			return {
				id,
				kind,
				label,
				...
				route,
				tools: unique$1(surfaces.flatMap((surface) => surface.toolNames)),
				systemHashes: surfaces.length > 0 ? unique$1(surfaces.map((surface) => surface.systemHash)) : fallback === void 0 ? [] : [fallback.systemHash],
				toolSchemaHashes: surfaces.length > 0 ? unique$1(surfaces.map((surface) => surface.toolSchemaHash)) : fallback === void 0 ? [] : [fallback.toolSchemaHash],
				...evidence?.metrics === void 0 ? {} : { metrics: evidence.metrics },
				...evidence?.callEvidence === void 0 ? {} : { callMetrics: evidence.callEvidence.metrics }
			};
		}
		function retainedExperiments(experiment, history) {
		...
		function buildDashboardPayload(replayCase, experiment, cohort = {}) {
			const variantLabels = new Map((cohort.variants ?? []).map((variant) => [variant.id, variant.label]));
			const baseline = slice(`observed-${replayCase.sourceSessionId}-${replayCase.sourceTurn}`, "baseline", "Observed baseline", experiment.baseline ?? replayCase.observedBaseline, replayCase);
			const candidate = slice(experiment.id, "candidate", variantLabels.get(experiment.candidateVariantId) ?? "Candidate replay", experiment.candidate);
			const runs = [baseline, ...retainedExperiments(experiment, cohort.history ?? []).map((item) => slice(item.id, "candidate", variantLabels.get(item.candidateVariantId) ?? item.candidateVariantId, item.candidate))];
			return {
				...
				candidate,
				runs,
				...experiment.scorecard === void 0 ? {} : { scorecard: { rows: experiment.scorecard.rows } },
		...
		//#endregion
		//#region src/dashboard-prompts.ts
		const MAX_DASHBOARD_PROMPT_CHARS = 12e3;
		/** Prompt Send is independent of the frozen observed-turn route. */
		const EVIDENCE_PROMPT_PROVIDER = "deepseek-official";
		const EVIDENCE_PROMPT_MODEL = "deepseek-v4-pro";
		const DASHBOARD_PROMPT_OPTIONS = [
			{
				id: "overlay-all-runs",
				kind: "dashboard",
		...
		const EVIDENCE_PROMPT_OPTIONS = [...DASHBOARD_PROMPT_OPTIONS, SENTENCE_PROMPT_OPTION];
		function evidencePromptOption(id) {
			return EVIDENCE_PROMPT_OPTIONS.find((option) => option.id === id) ?? DASHBOARD_PROMPT_OPTIONS[0];
		}
		function isDashboardPromptId(value) {
			return typeof value === "string" && DASHBOARD_PROMPT_OPTIONS.some((option) => option.id === value);
		}
		function isEvidencePromptId(value) {
			return typeof value === "string" && EVIDENCE_PROMPT_OPTIONS.some((option) => option.id === value);
		}
		//#endregion
		//#region src/dashboard-sandbox.ts
		const DASHBOARD_CSP = "default-src 'none'; base-uri 'none'; connect-src 'none'; form-action 'none'; frame-src 'none'; img-src data:; media-src 'none'; object-src 'none'; style-src 'unsafe-inline'; script-src 'unsafe-inline'";
		const DASHBOARD_SANDBOX = "allow-scripts";
		function jsonForScript(value) {
			return JSON.stringify(value).replaceAll("<", "\\u003c").replaceAll(">", "\\u003e");
		}
		function assembleDashboardDocument(payload, fragment) {
...
${fragment}
		...
		const DEFAULT_DASHBOARD_FRAGMENT = `<style>
...
<\/script>`;
		...
		function routeMismatchLabel(value) {
		...
		function routeEvidenceFor(experiment, evidence) {
		...
		function RouteLineagePanel({ evidence }) {
			if (evidence.length === 0) return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
				className: "rld-session-muted",
				children: "No durable parent/child route decision is attached to this run."
			});
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("section", {
				className: "rld-route-lineage",
				"aria-label": "Parent child route evidence",
				children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("header", { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("h4", { children: "Parent / child route evidence" }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: "Durable request headers only" })] }), evidence.map((item) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("article", {
					"data-mismatch": String(item.routeMismatch),
					children: [
						/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
							className: "rld-route-lineage-title",
							children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("strong", { children: routeMismatchLabel(item.routeMismatch) }), /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("code", {
								title: `${item.parentSessionId} → ${item.childSessionId}`,
								children: [
									compactIdentifier(item.parentSessionId),
									" → ",
									compactIdentifier(item.childSessionId)
								]
							})]
						}),
						/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("dl", { children: [
							/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("dt", { children: "Expected parent route" }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("dd", { children: routeLabel(item.expectedParentRoute) })] }),
							/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("dt", { children: "Actual child route" }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("dd", { children: routeLabel(item.actualChildRoute) })] }),
							/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("dt", { children: "Lineage source" }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("dd", { children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)("code", { children: item.provenance.lineage }) })] }),
							/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("dt", { children: "Route source" }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("dd", { children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)("code", { children: "durable request/header" }) })] })
						] }),
						item.routeMismatch === null && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("p", {
							className: "rld-session-warning",
							role: "status",
							children: ["Cannot decide: ", item.missingReason ?? "durable route evidence is incomplete"]
						})
					]
				}, `${item.parentSessionId}:${item.childSessionId}`))]
			});
		}
		...
		function EvidenceSummary({ title, evidence }) {
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("section", {
				...
				children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("header", { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("h4", { children: title }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("strong", {
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("dt", { children: "Evidence hash" }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("dd", {
						title: evidence.evidenceHash,
						children: evidence.evidenceHash === void 0 ? "Unavailable" : compactIdentifier(evidence.evidenceHash)
					})] }),
					evidence.workspace?.checkpoint !== void 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("dt", { children: "Checkpoint" }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("dd", {
						title: evidence.workspace.checkpoint.checkpointHash,
						children: compactIdentifier(evidence.workspace.checkpoint.checkpointHash)
					})] }),
					evidence.workspace?.rollback !== void 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("dt", { children: "Replay files" }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("dd", { children: evidence.workspace.rollback.status === "restored" ? "Restored to checkpoint" : evidence.workspace.rollback.status })] })
		...
		function workspaceDriftNotice(drift) {
			return drift?.detected === true ? "Source workspace continued past the pre-turn S0 checkpoint. The candidate replayed from that snapshot; the source was not reverted." : void 0;
		}
		function WorkspaceDriftNotice({ drift }) {
		...
		function EvidenceDashboardPanel({ replayCase, experiment, history, variants, rendering, summarizing, onRender, onSummarize }) {
			...
			const initialPromptOption = evidencePromptOption(isEvidencePromptId(dashboard?.promptId) ? dashboard.promptId : void 0);
			const [pendingPromptId, setPendingPromptId] = (0, react.useState)();
			const [selectedPromptId, setSelectedPromptId] = (0, react.useState)(initialPromptOption.id);
			const [promptDraft, setPromptDraft] = (0, react.useState)(dashboard?.prompt ?? initialPromptOption.instruction);
			const selectedOption = evidencePromptOption(selectedPromptId);
			const srcdoc = assembleDashboardDocument(payload, dashboard?.status === "completed" && dashboard.fragment !== void 0 ? dashboard.fragment : DEFAULT_DASHBOARD_FRAGMENT);
			const generated = dashboard?.status === "completed";
			const launched = dashboard?.promptId !== void 0 ? evidencePromptOption(dashboard.promptId) : void 0;
			const pendingOption = pendingPromptId === void 0 ? void 0 : evidencePromptOption(pendingPromptId);
			const generating = (rendering || summarizing) && pendingOption !== void 0;
			...
			const sentenceReady = experiment.callEvidenceComparison !== void 0;
			const sendDisabled = busy || promptDraft.trim().length === 0 || selectedOption.kind === "sentence" && !sentenceReady;
			(0, react.useEffect)(() => {
				if (!busy) setPendingPromptId(void 0);
			}, [busy]);
			(0, react.useEffect)(() => {
				if (dashboard?.prompt !== void 0 && isDashboardPromptId(dashboard.promptId)) {
					setSelectedPromptId(dashboard.promptId);
					setPromptDraft(dashboard.prompt);
				}
			}, [dashboard?.prompt, dashboard?.promptId]);
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("section", {
				className: "rld-result-section rld-evidence-narrative",
				"aria-labelledby": "rld-evidence-dashboard-heading",
				children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("header", { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("h3", {
					id: "rld-evidence-dashboard-heading",
					children: "Evidence summary"
				}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: generating ? `Sending prompt to ${EVIDENCE_PROMPT_MODEL} · reasoning off` : generated ? `${launched?.label ?? "Model chart"} · host-injected numbers` : `${saved} saved ${saved === 1 ? "run" : "runs"} · Saved runs highlights the series` })] }), /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
					className: "rld-disclosure-body",
					children: [
						/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
							className: "rld-dashboard-stage",
							"data-generating": generating || void 0,
							children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("iframe", {
								...
								srcDoc: srcdoc
							}), generating && pendingOption !== void 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
								className: "rld-dashboard-generating",
								role: "status",
								"data-testid": "dashboard-prompt-inflight",
								children: [
									...
									/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: pendingOption.label }),
									/* @__PURE__ */ (0, react_jsx_runtime.jsx)("small", { children: pendingOption.kind === "sentence" ? "One cited Chinese sentence. Host still supplies the evidence facts." : "Direct model-runtime generation; one contract-repair retry may follow. The host injects every number." })
								]
							})]
						}),
						dashboard?.status === "failed" && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("p", {
							className: "rld-session-warning",
							role: "status",
							children: ["Model dashboard rejected; showing the stable host fallback. ", dashboard.error]
						}),
						/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("p", {
							className: "rld-result-neutral-note",
							children: [
								"Prompt goes to ",
								EVIDENCE_PROMPT_PROVIDER,
								"/",
								EVIDENCE_PROMPT_MODEL,
								" with reasoning off. Numbers stay host-injected; the iframe cannot reach the parent session."
							]
						}),
						/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
							className: "rld-dashboard-prompts",
							children: [
								/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("header", { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("h4", { children: "Prompt" }), /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", { children: [EVIDENCE_PROMPT_MODEL, " · reasoning off · one shot, no agent"] })] }),
								/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
									className: "rld-prompt-presets",
									role: "tablist",
									"aria-label": "Prompt presets",
									children: EVIDENCE_PROMPT_OPTIONS.map((option) => /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
										type: "button",
								...
								/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
									...
									children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("textarea", {
									...
									}), /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("footer", { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", { children: [
										promptDraft.length.toLocaleString(),
										" / ",
										MAX_DASHBOARD_PROMPT_CHARS.toLocaleString()
									] }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
										className: "rld-prompt-actions",
										children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
											...
											children: pendingPromptId === selectedPromptId ? "Sending…" : "Send"
										})
									})] })]
								})
							]
						}),
						narrative?.text !== void 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("blockquote", { children: narrative.text }),
						narrative !== void 0 && narrative.citedEvidenceIds.length > 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("p", {
							className: "rld-behavior-note",
							children: ["Cited facts: ", narrative.citedEvidenceIds.join(", ")]
						})
					]
				})]
			});
		...
		function CompletedResult({ replayCase, experiment, activeExperiment, variants, history, routeLineage, workspaceDrift, rendering, summarizing, onPlan, onRender, onSummarize, onSelectHistory }) {
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("main", {
				...
				children: [
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)(EvidenceDashboardPanel, {
						replayCase,
						experiment,
						history,
						variants,
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("details", {
						...
						children: [
							/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("summary", { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("strong", { children: "Host comparison tables" }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: "Deterministic request surface · execution delta" })] }),
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("details", {
						...
						children: [
							...
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)(RouteLineagePanel, { evidence: routeEvidenceFor(experiment, routeLineage) })
						]
					})
				]
			});
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [
					...
					completedResult ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)(CompletedResult, {
						...
						onRender: (promptId, prompt) => {
							controller.renderDashboard(displayedExperiment.id, promptId, prompt);
						},
						onSummarize: (prompt) => {
							controller.summarize(displayedExperiment.id, prompt);
						},
...
.rld-route-lineage{padding:12px 18px;border-top:1px solid var(--rld-border);background:#fbfcfe}.rld-route-lineage>header{display:flex;align-items:center;justify-content:space-between;gap:12px;margin-bottom:8px}.rld-route-lineage>header span{color:var(--rld-muted);font-size:9px}.rld-route-lineage article{display:grid;grid-template-columns:190px minmax(0,1fr);gap:12px;padding:10px;border:1px solid var(--rld-border);border-left:3px solid #98a2b3;border-radius:5px;background:#fff}.rld-route-lineage article+article{margin-top:8px}.rld-route-lineage article[data-mismatch=true]{border-left-color:#d92d20}.rld-route-lineage article[data-mismatch=false]{border-left-color:#18a558}.rld-route-lineage-title{display:flex;min-width:0;flex-direction:column;gap:4px}.rld-route-lineage-title strong{font-size:11px}.rld-route-lineage article[data-mismatch=true] .rld-route-lineage-title strong{color:#b42318}.rld-route-lineage article[data-mismatch=false] .rld-route-lineage-title strong{color:#067647}.rld-route-lineage-title code{overflow:hidden;color:var(--rld-muted);font-size:8px;text-overflow:ellipsis;white-space:nowrap}.rld-route-lineage dl{display:grid;grid-template-columns:1fr 1fr;gap:6px 14px;margin:0}.rld-route-lineage dt{color:var(--rld-muted);font-size:8px}.rld-route-lineage dd{margin:1px 0 0;overflow:hidden;font:600 9px/1.4 ui-monospace,SFMono-Regular,Menlo,monospace;text-overflow:ellipsis;white-space:nowrap}.rld-route-lineage article>p{grid-column:1/-1;margin:0}
.rld-evidence-narrative .rld-disclosure-body{padding:16px}.rld-evidence-narrative blockquote{margin:12px 0 0;color:#27364b;font-size:12px;line-height:1.65}.rld-evidence-narrative .rld-behavior-note{padding:10px 0;text-align:left}.rld-dashboard-frame{display:block;width:100%;height:460px;border:1px solid var(--rld-border);border-radius:4px;background:#fff}.rld-dashboard-stage{position:relative}.rld-dashboard-stage[data-generating] .rld-dashboard-frame{filter:saturate(.55) brightness(.97)}.rld-dashboard-generating{position:absolute;inset:0;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:6px;overflow:hidden;border-radius:4px;background:linear-gradient(180deg,rgba(255,255,255,.78),rgba(238,244,255,.92));color:#1d4ed8;text-align:center;pointer-events:none}.rld-dashboard-generating::before{content:"";position:absolute;inset:0;background:linear-gradient(105deg,transparent 35%,rgba(36,95,218,.14) 50%,transparent 65%);background-size:220% 100%;animation:rld-prompt-scan 1.4s linear infinite}.rld-dashboard-generating strong,.rld-dashboard-generating span,.rld-dashboard-generating small{position:relative}.rld-dashboard-generating strong{font-size:12px}.rld-dashboard-generating span{color:#344054;font-size:11px}.rld-dashboard-generating small{max-width:28em;color:#667085;font-size:10px;line-height:1.45}.rld-evidence-narrative>.rld-disclosure-body>.rld-result-neutral-note{margin-top:8px;padding:0;border:0;background:transparent}
.rld-dashboard-prompts{margin-top:10px;padding-top:8px;border-top:1px solid #e9edf3}
.rld-dashboard-prompts>header{display:flex;align-items:baseline;justify-content:space-between;gap:10px;margin-bottom:6px}
.rld-dashboard-prompts h4{margin:0;font-size:11px}
.rld-dashboard-prompts>header span{color:var(--rld-muted);font-size:10px}
.rld-prompt-presets{display:flex;flex-wrap:wrap;gap:4px;margin-bottom:6px}
.rld-evidence-narrative button.rld-prompt-preset{display:inline-flex;align-items:center;min-height:22px;padding:0 8px;border:1px solid #d9dee8;border-radius:999px;background:#fff;color:#344054;font-size:10px;font-weight:650;line-height:1;cursor:pointer}
.rld-evidence-narrative button.rld-prompt-preset[aria-pressed=true]{border-color:#245fda;background:#eef4ff;color:#1d4ed8}
.rld-evidence-narrative button.rld-prompt-preset:disabled{cursor:not-allowed;opacity:.5}
.rld-prompt-composer{display:flex;flex-direction:column;border:1px solid #d9dee8;border-radius:6px;background:#fff;overflow:hidden}
.rld-prompt-composer:focus-within{border-color:#245fda}
.rld-prompt-composer textarea{box-sizing:border-box;width:100%;min-height:58px;max-height:140px;resize:vertical;padding:8px 10px 2px;border:0;background:transparent;color:#27364b;font:400 11px/1.45 ui-monospace,SFMono-Regular,Menlo,monospace}
.rld-prompt-composer textarea:focus{outline:none}
.rld-prompt-composer textarea:disabled{color:#667085}
.rld-prompt-composer>footer{display:flex;align-items:center;justify-content:space-between;gap:8px;padding:3px 6px 6px 10px}
.rld-prompt-composer>footer span{color:#667085;font-size:9px}
.rld-prompt-actions{display:flex;align-items:center;gap:4px}
.rld-evidence-narrative button.rld-prompt-send{min-height:22px;padding:0 8px;border:1px solid #245fda;border-radius:4px;background:#245fda;color:#fff;font-size:10px;font-weight:650;cursor:pointer}
.rld-evidence-narrative button.rld-prompt-send:disabled{opacity:.45;cursor:not-allowed}
@keyframes rld-prompt-scan{0%{background-position:120% 0}100%{background-position:-120% 0}}
@media(max-width:560px){.rld-route-lineage article{display:block}.rld-route-lineage dl{margin-top:10px}.rld-prompt-composer>footer{align-items:flex-end;flex-direction:column}}@media(prefers-reduced-motion:reduce){.rld-dashboard-generating::before{animation:none}}
